...
func NewCache(redis *redis.Client, repo repository.DynamicAnalysisRepository) *Cache {
	return &Cache{
		redis: redis,
		repo:  repo,
	}
...
func (c *Cache) Get(ctx context.Context, sourceHash string) (*dynamic_analysis.AnalyzeResponse, error) {
	// L1: Try Redis first
	if c.redis != nil {
...
func (c *Cache) Set(ctx context.Context, sourceHash string, result *dynamic_analysis.AnalyzeResponse) error {
	// Save to Database (L2)
